...
# Safety check to fail early with a clear message
if (!is.list(file_config) || is.null(file_config$sample_files)) {
  stop(paste0(
    "Failed to parse TOML configuration from path: ", params$config_file, 
...
get_log_file <- function(phase, config, sample, locus) {
  phase_found <- FALSE
  template_path <- NULL
  ...
  for (element in file_config$sample_files) {
    if (element[1] == phase) {
      ...
      template = stri_replace_all_fixed(template, "{locus}", locus)
...
quality_result = get_log_file('filterSeq_quality', file_config, params$sample, params$locus)
length_result = get_log_file('filterSeq_length', file_config, params$sample, params$locus)
primer_result_1 = get_log_file('maskPrimer_C', file_config, params$sample, params$locus)
primer_result_2 = get_log_file('maskPrimer_V', file_config, params$sample, params$locus)
align_result = get_log_file('alignSets', file_config, params$sample, params$locus)
total_result = get_log_file('total', file_config, params$sample, params$locus)
unique_result = get_log_file('unique', file_config, params$sample, params$locus)
atleast2_result = get_log_file('unique_atleast2', file_config, params$sample, params$locus)
